...
private:
  int map_x_;// xác định giới hạn của nó với bản đồ
  int map_y_;
  float x_val_;// cái vị trí mà nó thay đổi nó sẽ tăng lên
  float y_val_;
  bool is_alive_;
  bool is_stop_bullet_;
  float x_pos_;// vị trí nó đang nằm 
  float y_pos_;
  int on_ground_;
  int think_time_;//thời gian trở lại khi rơi xuong vực thẳm
  Input input_type_;
  SDL_Rect frame_clip_[NUM_FRAME];
  int width_frame_;
  int height_frame_;
  int frame_;
  int animation_a_;//giới hạn tay trái
  int animation_b_;//giới hạn bên tay phải
  int type_move_;

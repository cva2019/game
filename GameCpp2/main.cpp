...
std::vector<ThreatsObject*> MakeThreadList()
{
    ...
    ThreatsObject* thread_objs = new ThreatsObject[30];
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        if (thread_obj != NULL)
        {
            ...
            BulletObject* p_bullet = new BulletObject();
             thread_obj->InitBullet(p_bullet, g_screen);
...
int main(int argc, char* argv[])
{
    ...
    GameMap game_map;
    game_map.LoadMap("map//map01.dat ");//THAY ĐỔI
    ...
    while(!quit)
    {
       ...
       //Draw Geometric
       GeometricFormat rectange_size(0, 0, SCREEN_WIDTH, 40);// xóa
